...
- [x] Manual: send a recipe URL → verify parsed recipe confirmation flow still works
- [x] Manual: save recipe → `SELECT embedded FROM recipes ORDER BY id DESC LIMIT 1;` → should be `1`
- [x] Manual: restart bot with an unembedded recipe → console logs `"reconciling 1 unembedded recipe(s)..."`
